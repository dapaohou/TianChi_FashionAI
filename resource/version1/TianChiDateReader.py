...
# 2.返回的图片为float，返回的landmarks/visibilities为int，需要在tensorflow以int形式接收，然后转换为float
#
# 20180314做了如下修改：
# 1.专为DAN这种分stage的训练方式，添加了参数stage，stage=0则数据从csv读取并打乱，stage!=0则从npy文件读取数据。
#   这样做的好处是能够保证所有stage使用的train集和val集相同，以免出现stage=1时的val集数据，在stage=2时跑到train集
"""
...
class TianChiDateReader(object):
    def __init__(self, data_dir, stage, img_size=512, landmarks_num=24, train_size=0.8):
        ...
        if(stage == 0 or stage is None):
            self.read_csv()
            self.shuffle_data()
            self.merge_landmark()
            self.save_npy_files()    # stage = 0, save npy
        else:
            self.load_npy_files()    # stage != 0, load npy
    ...
    def save_npy_files(self):
        np.save(os.path.join(self.data_dir, "images_path.npy"), self.images_path)
        np.save(os.path.join(self.data_dir, "landmarks_merged.npy"), self.landmarks_merged)
        np.save(os.path.join(self.data_dir, "visibilities_merged.npy"), self.visibilities_merged)

    def load_npy_files(self):
        self.images_path = np.load(os.path.join(self.data_dir, "images_path.npy"))
        self.landmarks_merged = np.load(os.path.join(self.data_dir, "landmarks_merged.npy"))
        self.visibilities_merged = np.load(os.path.join(self.data_dir, "visibilities_merged.npy"))
...
if __name__ == "__main__":
    """用法说明：
    # 0.首先确保本文件与主文件在同一目录下，主文件添加 from TianChiDateReader import TianChiDateReader
    #
    # 1.声明一个“天池数据读取器”，"../train_modified"为我电脑上的数据路径，train_size=0.8代表有80%的数据用于train,20%数据用于validate
    #   stage专用于DAN这种分stage的训练方式。对于不分stage的网络，则设stage=0，注意，声明不要放在for循环内
    # tr = TianChiDateReader("../train_modified", stage=0, img_size=512, train_size=0.8)
    #
